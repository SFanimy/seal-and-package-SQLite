...
- (void)fl_insertModel:(id)model complete:(void(^)(FLFMDBQueueManager *manager, BOOL flag))complete{
    [self fl_insertModel:model autoCloseDB:NO complete:complete];
}
...
- (void)fl_searchRequireArr:(Class)modelClass byRequireKey:(NSString *)key RequireValue:(NSString *)value complete:(void (^)(FLFMDBQueueManager *, NSArray *))complete{
    [self fl_searchRequireArr:modelClass byRequireKey:key RequireValue:value autoCloseDB:YES complete:complete];
}
 ...
 *  创建插入表的SQL语句
 */
- (NSString *)createInsertSQL:(id)model dbClass:(FMDatabase *)db{// OR REPLACE
    ...
    for (int i = 0; i < outCount; i ++) {
        ...
        //把model中新增属性，插入表单
        for (int i =0; i <outCount; i++) {
            if (![db columnExists:key inTableWithName:[NSString stringWithFormat:@"%@",[model class]]]) {
                
                NSString *alertStr = [NSString stringWithFormat:@"alter table %@ add %@ ",[model class],key];
...
//            }
            ...
            if (success) {
                ...
                if ([[judgeModle valueForKey:@"FLDBID"] isEqualToString:fl_dbid]) {
                    ...
                    if (insertSuccess) {
                        insertSuccess = [db executeUpdate:[self createInsertSQL:model dbClass:db]];
                    }
...
//            }
            ...
            if ([[judgeModle valueForKey:@"FLDBID"] isEqualToString:fl_dbid]) {
                ...
                BOOL insertSuccess = [db executeUpdate:[self createInsertSQL:model dbClass:db]];
                // 最后一步操作完毕，询问是否需要关闭
                if (autoCloseDB) {
                    [self fl_closeDB:db];
                }
...
- (void)fl_searchRequireArr:(Class)modelClass byRequireKey:(NSString *)key RequireValue:(NSString *)value autoCloseDB:(BOOL)autoCloseDB complete:(void(^)(FLFMDBQueueManager *manager,id model))complete{
    __block BOOL success = true;
    __weak typeof(self) weakSelf = self;
    //把任务包装到事务里
    FLDISPATCH_ASYNC_GLOBAL(^{
        [FLCURRENTDBQUEUE inTransaction:^(FMDatabase *db, BOOL *rollback){
            ...
            if (complete) {
                FLDISPATCH_ASYNC_MAIN(^{
                    complete(strongSelf,[strongSelf fl_search:db model:modelClass byRequireKey:key RequireValue:value autoCloseDB:autoCloseDB]);
                });
            }
...
- (id)fl_search:(FMDatabase *)db model:(Class)modelClass byRequireKey:(NSString *)key RequireValue:(NSString *)value autoCloseDB:(BOOL)autoCloseDB{
    ...
    if ([db open]) {
        ...
        NSString *str = [NSString stringWithFormat:@"SELECT * FROM %@ WHERE %@ = '%@';",modelClass,key,value];
