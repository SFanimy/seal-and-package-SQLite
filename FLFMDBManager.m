...
- (NSArray *)fl_searchRequireArr:(Class)modelClass byRequireKey:(NSString *)key RequireValue:(NSString *)value{
    return [self fl_searchModel:modelClass byRequireKey:(NSString *)key RequireValue:(NSString *)value autoCloseDB:YES];
}
...
- (NSString *)createInsertSQL:(id)model{
    ...
    for (int i = 0; i < outCount; i ++) {
        ...
        //把model中新增属性，插入表单
        for (int i =0; i <outCount; i++) {
            if (![FLCURRENTDB columnExists:key inTableWithName:[NSString stringWithFormat:@"%@",[model class]]]) {
                
                NSString *alertStr = [NSString stringWithFormat:@"alter table %@ add %@ ",[model class],key];
                BOOL result = [FLCURRENTDB executeUpdate:alertStr];
                if (result == YES) {
                    NSLog(@"Success");
                }
            }
...
- (NSArray *)fl_searchModel:(Class)modelClass byRequireKey:(NSString *)key RequireValue:(NSString *)value autoCloseDB:(BOOL)autoCloseDB{
    if ([FLCURRENTDB open]) {
//        FL_ISEXITTABLE(modelClass);
        if(![self isExitTable:modelClass autoCloseDB:NO]){
            if (autoCloseDB) {
        ...
        // 查询数据
        FMResultSet *rs = [FLCURRENTDB executeQuery:[NSString stringWithFormat:@"SELECT * FROM %@ WHERE %@ = '%@';",modelClass,key,value]];
         NSMutableArray *modelArrM = [NSMutableArray array];
        // 创建对象
        id object = nil;
        // 遍历结果集
