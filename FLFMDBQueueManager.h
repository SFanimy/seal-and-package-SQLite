...
/**
 ...
 @param key 条件键值对的键
 @param value 条件键值对的值
 @param complete 不等于nil，表示查询数据操作执行成功并有数据，返回查询成功的模型数据，nil则表示查询操作失败 或者 查询成功但数据为空 或者 对应的表格不存在
 */

- (void)fl_searchRequireArr:(Class)modelClass byRequireKey:(NSString *)key RequireValue:(NSString *)value complete:(void(^)(FLFMDBQueueManager *manager,NSArray *array))complete;
/**
 *  @author gitKong
 *
 *  查找指定表中模型数组（所有的）
